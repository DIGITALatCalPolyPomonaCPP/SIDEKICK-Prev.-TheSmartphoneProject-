# Contributors and source owners

This index combines author metadata from the imported source repositories with additional contributor attribution supplied by the project maintainer. Repository-derived authorship remains documented at commit level in each subsystem's `PROVENANCE.md`.

| Contributor | GitHub identity | Recorded workstream(s) |
| --- | --- | --- |
| SunnyYoshimitsu | [@SunnyYoshimitsu](https://github.com/SunnyYoshimitsu) | System on Module (FPGA Main Board) and Peripheral Daughter Board |
| Max Gross | [@pixelatedknight27](https://github.com/pixelatedknight27) | System on Module (FPGA Main Board) and Peripheral Daughter Board |
| Ethan Oliver Wong | [@CondescendingCottageCow](https://github.com/CondescendingCottageCow) | FPGA main board and peripheral board |
| Anish Chinnakonda | [@anishc-AC](https://github.com/anishc-AC) | Haptics and fingerprint sensor |
| Angelo Duenas | [@AngeloDuenas](https://github.com/AngeloDuenas) | Haptics and fingerprint sensor |
| Ethan Trone | [@the-bumbler](https://github.com/the-bumbler) | Flashlight Driver, FIFO Buffer Logic Design, AXI BUS Logic Design|
| Logan Knip | [@llknip](https://github.com/llknip) | Mechanical enclosure work and ambient-light sensor |
| Russell Chung | [@ARussellChung](https://github.com/ARussellChung) | Product Manager, Project Manager, Systems Engineering Director, Mechanical Phone Models, Fingerprint Sensor, Thermometer, Flashlight |
| Sebastian Salgado | [@Chiparino](https://github.com/Chiparino) | Trickle Charge. Power Management Module |
| Mason Tran | [@studentmtran-lab](https://github.com/studentmtran-lab) | IMU board iterations; source commit identity `masontran44` |
| Oscar Cao |[@CondescendingCottageCow](https://github.com/CondescendingCottageCow) | IMU board iterations |
| Eryn Chen | [@eryn-chen](https://github.com/eryn-chen) | Power Management Module |
| Allen Nguyen | [@Allen2331](https://github.com/Allen2331) | IMU, FIFO Buffer Logic Design, AXI BUS Logic Design |
| Viren Kumar | [@vurbe](https://github.com/vurbe) | Mechanical/thermal repository and enclosure work |
| Ivan Aguilar-Moreno | [@aguilarmorenoivan](https://github.com/aguilarmorenoivan) |Light Sensor |
| Lisette Santana | Profile not supplied | Light Sensor |
| DIGITAL at Cal Poly Pomona | [@DIGITALatCalPolyPomonaCPP](https://github.com/DIGITALatCalPolyPomonaCPP) | IMU repository and project stewardship |

The mechanical snapshot also contains files whose filenames attribute designs to Izaiah Ortiz and Russell. Those file-level credits are retained exactly as they appeared in the source repository.

For exact commit authorship and source links, see the `PROVENANCE.md` file in each subsystem directory.
